...
function AddNotice({ closeModal, setMyNotices, editNotice }) {
    ...
    const addOrEditNotice = async () => {
        ...
        if (editNotice) {
            ...
            setMyNotices((prev) =>
                prev.map((notice) => (notice.id === addedOrUpdatedNotice.id ? addedOrUpdatedNotice : notice))
            );
        } else {
    ...
    return (
        <div className="addNotice__overlay">
            <div className="addNotice__modalContainer">
                <h3 className="addNotice__modalHeader">{editNotice ? "Edit Notice" : "Add a new notice"}</h3>
                <form>
                    ...
                    <div className="addNotice__buttonContainer">
                        <button type="button" onClick={addOrEditNotice} className="addNotice__button">
                            {editNotice ? "Update Notice" : "Add Notice"}
                        </button>
                        <button type="button" onClick={closeModal} className="addNotice__closeButton">
                            Close
                        </button>
                    </div>
                </form>
